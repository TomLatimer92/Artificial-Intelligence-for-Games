    ...
    for key, value in actions.items(): 
        ...
        if best_goal in value:
            ...
            ### 2. use the "action_utility" function to find the best_utility value of this best_action
                ...
                # action_utility(best_utility == best_action);
            ...
            ### 1. use the "action_utility" function to find the utility value of this action
                # action_utility(utility_value, action);
            ...
            ### 2. If it's the best action to take (utility > best_utility), keep it! (utility and action)
                if (utility > best_utility):
                    ...
                    best_action = key;
##                elif best_utility < action_utility:
...
def retrieve_discontent(action, goal_list):
	discontent = 0
	for goal, change in list(actions[action].items()):
            # Adds discontent to the list of goals and changes.
	    temporary = max(goal_list[goal]+change, 0)
	    discontent += temporary*temporary
	return discontent

def preferredAction(goal_list):
	best_discontent = 99999
	best_action = None
	for key, value in actions.items():     
	    currentDis = retrieve_discontent(key, goal_list)
	    print(key, '(', current_discontent, ')')
	    if currentDis < best_discontent:
		    best_discontent = currentDis
		    best_action = key
	return best_action
...
def choice(maxDepth):
	...
	for key, value in actions.items():
		...
		print('level of discontent is:', run_discontent)
		best_action = key
		...
		if depth > 0:
			...
			for key, value in actions.items():
				...
				run_discontent = retrieve_discontent(key,temporary)
				print('level of discontent is: ', run_discontent)
				print('new goals:', temporary)
				depth -= 1
				if depth > 0:
					...
					for key, value in actions.items():
						...
						run_discontent = retrieve_discontent(key,goals)
						print('level of discontent is:', run_discontent)
						print('new goal:', temporary)

						print('                ---> running:', run_discontent, 'best:', best_discontent)
						if run_discontent < best_discontent:
							best_discontent = run_discontent
