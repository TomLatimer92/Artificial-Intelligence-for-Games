...
def SearchDFS(graph, source_idx, target_idx, limit=0):
    ...
    route = {} 
	# dict of {to:from} items to find our way home.
    open = [] 
	# use a list as a LIFO stack of the current leaf edges.
    steps = 0 # if limit
    ...
    while len(open):
        steps += 1
        leaf = open.pop() 
		# get the last added (LIFO) edge to investigate.
        closed.add(leaf) 
		# set as 'visited'.
        if leaf == target_idx:
            break
...
def SearchBFS(graph, source_idx, target_idx, limit=0):
	...
	# dict of {to:from} items to find our way home.
    open = [] 
	# use a list as a FIFO queue of the current leaf edges.
    steps = 0 
	# if limit.
    # add the starting source as an edge tuple to self.
    open.append( source_idx )
    ...
    while len(open):
        steps += 1
        leaf = open.pop(0) 
		# get's the first (FIFO) node to investigate.
        closed.add(leaf)
        if leaf == target_idx:
            break
        else:
...
def SearchDijkstra(graph, source_idx, target_idx, limit=0):
	...
	# dict of {to:from} items to find our way home.
    open = PriorityQueue() 
	# priority queue of the current leaf edges.
    steps = 0 
	# if limit.
    ...
    while len(open):
        steps += 1
        leaf, cost = open.pop() 
		# get the lowest cost-so-far node to investigate.
        closed.add(leaf)
        if leaf == target_idx:
            break
        else:
            idxs = graph.get_neighbours(leaf)
            for dest in idxs:
                if dest not in closed: 
				# visited.
                    cost_f = cost + graph.get_edge(leaf,dest).cost 
					# cost_g.
                    if dest in open: 
					# old path to same node?
                        if open.peek(dest)[1] <= cost_f: 
						# if better, keep it.
                            continue
                        else: 
						# remove the old, and the new one be added.
                            open.remove(dest)
                    route[dest] = leaf 
					# to:from.
    ...
    while len(open):
        ...
        if leaf == target_idx:
            ...
            for dest in idxs:
                if dest not in closed: 
				# visited.
                    cost_g = cost + graph.get_edge(leaf, dest).cost 
					# G cost-so-far.
                    cost_h = graph.cost_h(dest, target_idx) 
					# H estimated-cost.
